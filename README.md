...
| File | What it is |
|---|---|
| `Morgan_Recap_Analysis.pdf` | 5 branded, **table‑driven** pages. **p1** portfolio data tape (with each asset's **market / metro**) + NOI‑bridge / tax / economic‑loss table; then **one page per deal** that flows top‑to‑bottom: **NOI & cap‑rate bridge** (GPR → loss‑to‑lease → **AGPR** → vacancy/bad‑debt/concessions → NRI → EGR → opex → NOI, columns T12 / T3 / JLL UW / **per‑unit**, with **guidance price and cap rate (NOI ÷ price) as rows under each NOI column**, and the forward caps + NOI walk in the note) · **Market Rent (HelloData executed) & economic‑loss trend** (T12/T3/JLL UW, with executed‑lease sample **n**) · **unit mix** (contract rent vs HD‑executed market, HD **n**, new/renewal counts, gross & effective trade‑outs) · **resident demographics**. Money is formatted consistently throughout (**$X.XXM / $XXXK**, per‑unit annual whole‑$, accounting parens for contras). |
